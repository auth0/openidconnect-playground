...
export const saveUTM = () => {
  ...
  const sessionUtmValues: Record<string, string> = {};
  const originalUtmCookie = getCookie("_okta_original_attribution");
  const originalUtmValues: Record<string, string> = {};
  ...
  const gclidCookie = urlParams.get(GCLID_COOKIE);

  if (gclidCookie && !getCookie(GCLID_COOKIE)) {
    setCookie(GCLID_COOKIE, gclidCookie, getExpiryInDays(365));
  }
};
